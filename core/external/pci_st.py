...
def calc_PCIst(signal_evk, times, full_return=False, **par):
    ...
    **pars : dictionary
    ...
    signal_evk, times = preprocess_signal(
        signal_evk, times, (par["baseline_window"][0], par["response_window"][1]), **par
    )
    signal_svd, var_exp, eigenvalues, snrs = dimensionality_reduction(
        signal_evk, times, **par
    )
    ...
    if full_return:
        return {
            "PCI": PCI,
            **STQ,
            "signal_evk": signal_evk,
            "times": times,
            "signal_svd": signal_svd,
...
def dimensionality_reduction(
    signal, times, response_window, max_var=99, min_snr=1.1, n_components=None, **kwargs
):
...
def state_transition_quantification(
    signal,
    times,
    k,
    baseline_window,
    ...
    **kwargs
    ...
    for i in range(n_dims):
        D_base[i, :, :] = recurrence_matrix(
            baseline[i, :, :], thr=None, mode="distance"
        )
        D_resp[i, :, :] = recurrence_matrix(
            response[i, :, :], thr=None, mode="distance"
        )
...
def preprocess_signal(
    ...
    **kwargs
    ...
    assert signal_evk.shape[1] == len(
        times
    ), "Signal and Time arrays must be of the same size."
    if avgref:
...
def undersample_signal(signal, times, new_fs):
    ...
    new_signal_evk, new_times = scipy.signal.resample(
        signal, n_samples, t=times, axis=1
    )
...
def baseline_correct(Y, times, delta=0):
    ...
    close_enough = np.all(
        np.isclose(np.mean(newY[:, :onset_ix], axis=1), 0, atol=1e-08)
    )
